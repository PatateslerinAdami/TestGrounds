...
public class SwainShadowGrasp: ISpellScript {
    ...
    public void OnSpellPreCast(ObjAIBase owner, Spell spell, AttackableUnit target, Vector2 start, Vector2 end) {
        _position = end;
        _spell    = spell;
        AddParticle(_swain, null, "Swain_shadowGrasp_warning_green", _position, 1f, 1f, enemyParticle: "Swain_shadowGrasp_warning_red");
    ...
    public void OnUpdate(float diff) {
        ...
        if (ticks != 1) return;
        var units = GetUnitsInRange(_swain ,_position, 625f, true, SpellDataFlags.AffectEnemies | SpellDataFlags.AffectNeutral | SpellDataFlags.AffectHeroes | SpellDataFlags.AffectMinions);

        var ap = _swain.Stats.AbilityPower.Total * _spell.SpellData.Coefficient;
        var dmg = 80f + 40 * (_spell.CastInfo.SpellLevel - 1) + ap;
        foreach (var unit in units) {
                unit.TakeDamage(_swain, dmg, DamageType.DAMAGE_TYPE_MAGICAL, DamageSource.DAMAGE_SOURCE_SPELLAOE, DamageResultType.RESULT_NORMAL);
