...
public class SwainDecrepify : IBuffGameScript {
    ...
    public void OnUpdate(float diff) {
        ...
        if (ticks != 1) return;
        var target = GetUnitsInRange(_swain, _unit.Position, 625f, true, SpellDataFlags.AffectEnemies | SpellDataFlags.AffectHeroes | SpellDataFlags.AffectMinions | SpellDataFlags.AffectNeutral)
                     .OrderBy(unit => unit is Champion).ThenBy(unit => Vector2.Distance(_unit.Position, unit.Position)).FirstOrDefault();
        if (target != null) {
            FaceDirection(target.Position, _unit, true);
            RemoveParticle(_p);
            RemoveParticle(_p1);
            RemoveParticle(_p2);
            _p = AddParticleTarget( _unit, _unit, "swain_disintegrationBeam_beam", target, 1f, bone:"Bird_head", targetBone: "head");
            ...
            target.TakeDamage(
                ...
                _swain.Stats.AbilityPower.Total * _swain.GetSpell("SwainDecrepify").SpellData.Coefficient, DamageType.DAMAGE_TYPE_MAGICAL,
