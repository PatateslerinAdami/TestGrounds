...
public class SwainMetamorphism : ISpellScript {
    ...
    public void OnActivate(ObjAIBase owner, Spell spell) { _swain = owner; }
    ...
    private void ApplyBirds() {
        var closestUnits = GetUnitsInRange(_swain, _swain.Position, 700f, true, SpellDataFlags.AffectEnemies | SpellDataFlags.AffectNeutral | SpellDataFlags.AffectHeroes | SpellDataFlags.AffectMinions)
                           .OrderBy(unit => unit is Champion).ThenBy(unit => unit.HasBuff("SwainTorment")).ThenBy(unit => unit is Minion)
