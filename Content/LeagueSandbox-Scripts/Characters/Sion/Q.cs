...
namespace Spells
{
    public class SionQ : ISpellScript
    {
        ...
        public void OnSpellChannel(Spell spell)
        {
            ...
            AddParticlePos(_owner, "Sion_Base_Q_Cas.troy", _owner.Position, GetPointFromUnit(_owner, 1000f), lifetime: 1, direction: _owner.Direction);
            p1 = AddParticleTarget(_owner, _owner, "sion_base_q_cas1_weapon.troy", _owner, lifetime: 5, bone: "Buffbone_Glb_Weapon_1");
            p2 = AddParticlePos(_owner, "Sion_Base_Q_Indicator2.troy", _owner.Position, default, lifetime: 1, direction: _owner.Direction);
        }
        ...
        public void OnUpdate(float diff)
        {
            if (channeling)
            {
                ...
                if (currentCharge >= 1.0f && !passedOneSecond)
                {
                    ...
                    p3 = AddParticlePos(_owner, "Sion_Base_Q_Indicator_Red2.troy", _owner.Position, default, lifetime: 1, direction: _owner.Direction);
                    p4 = AddParticleTarget(_owner, _owner, "sion_base_q_cas2_weapon.troy", _owner, lifetime: 5, bone: "Buffbone_Glb_Weapon_1");
                    p5 = AddParticlePos(_owner, "sion_base_q_cas3.troy", _owner.Position, GetPointFromUnit(_owner, 1000f), lifetime: 1, direction: _owner.Direction, size: 1.7f);
                }
            }
        }
        ...
        private void LetGo(Spell spell, ChannelingStopSource reason)
        {
            ...
            if (currentCharge >= 1.0f)
            {
                ...
                AddParticlePos(_owner, "sion_base_q_hit3.troy", _owner.Position, GetPointFromUnit(_owner, currentRange), lifetime: 1, direction: _owner.Direction);
            ...
            else
            {
                ...
                CreateCustomMissile(_owner, "SionQHitParticleMissile2", _owner.Position, GetPointFromUnit(_owner, currentRange - 200f), new MissileParameters { Type = MissileType.Arc });
            }

            Vector2 dir = Vector2.Normalize(new Vector2(_owner.Direction.X, _owner.Direction.Z));
            if (dir == Vector2.Zero) dir = new Vector2(1, 0);

            foreach (var unit in GetUnitsInRange(_owner.Position, currentRange + 200f, true))
            {
                if (unit.Team == _owner.Team || !unit.GetIsTargetableToTeam(_owner.Team)) continue;
            ...
            _owner.RegisterTimer(new GameScriptTimer(0.25f, () =>
            {
                _owner.SetStatus(StatusFlags.CanMove, true);
            }));
        }
