...
namespace Buffs
{
    internal class SionWShieldStacks : IBuffGameScript
    {
        ...
        public void OnActivate(AttackableUnit unit, Buff buff, Spell ownerSpell)
        {
            ...
            _soundLoop = SpellEffectCreate("Sion_Base_W_Sound.troy", _sion, _sion, _sion,
                lifetime: _buff.Duration + 0.5f, scale: 1.3f, boneName: "C_Buffbone_Glb_Center_Loc",
                flags: FXFlags.SimulateWhileOffScreen);
            _shieldParticle = SpellEffectCreate("Sion_Base_W_Shield.troy", _sion, _sion, _sion,
                lifetime: _buff.Duration, scale: 0.5f, boneName: "C_Buffbone_Glb_Center_Loc",
                flags: FXFlags.SimulateWhileOffScreen);

            var hpScaling = _sion.Stats.HealthPoints.Total *
                ownerSpell.SpellData.EffectLevelAmount[6][ownerSpell.CastInfo.SpellLevel] / 100;
            var ap = _sion.Stats.AbilityPower.Total * ownerSpell.SpellData.Coefficient;
            var shieldValue = ownerSpell.SpellData.EffectLevelAmount[1][ownerSpell.CastInfo.SpellLevel] + ap +
                              hpScaling;
        ...
        public void OnDeactivate(AttackableUnit unit, Buff buff, Spell ownerSpell)
        {
            ...
            {
                if (!HasBuff(_sion, "SionQ"))
                {
                    PlayAnimation(_sion, "Spell2_B", 0, 0, 1,
                        AnimationFlags.NoBlend | AnimationFlags.Junk5 | AnimationFlags.Junk7);
                }

                SpellEffectCreate("Sion_Base_W_Nova.troy", _sion, null, null, _sion.Position, _sion.Position,
                    scale: 1.3f, flags: FXFlags.SimulateWhileOffScreen);
                ...
                {
                    var maxHealthDmg = unitInRange.Stats.HealthPoints.Total *
                        (ownerSpell.SpellData.EffectLevelAmount[6][ownerSpell.CastInfo.SpellLevel] / 100) + dmg;

                    SpellEffectCreate("Sion_Base_W_Nova_Tar.troy", _sion, null, null, unitInRange.Position,
                        _sion.Position, scale: 1.3f, orientTowards: unitInRange.GetPosition3D(),
