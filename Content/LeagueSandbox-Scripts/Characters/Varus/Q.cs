...
public class VarusQMissile : ISpellScript
{
    ...
    public SpellScriptMetadata ScriptMetadata { get; private set; } = new SpellScriptMetadata
    {
        ...
        {
            // Arc per VarusQMissile.json CastType 3 (line missile). The charge-system flow
            // now spawns the missile FROM this metadata (SpellCastCharge), so unlike the old
            // CreateCustomMissile path this value is live — Circle would wrongly make it an
            // orbit missile.
